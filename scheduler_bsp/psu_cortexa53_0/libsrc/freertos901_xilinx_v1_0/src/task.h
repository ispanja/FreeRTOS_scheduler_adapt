 ...
 */
	...
	//Modification of xTaskCreate()function for EDF algorithm
	...
	//Modification of xTaskCreat() function for LLF algorithm
#if ( configUSE_LLF_SCHEDULER == 1 )
	BaseType_t xTaskLlfCreate(	TaskFunction_t pxTaskCode,
									const char * const pcName,		/*lint !e971 Unqualified char types are allowed for strings and single characters only. */
									const configSTACK_DEPTH_TYPE usStackDepth,
									void * const pvParameters,
									UBaseType_t uxPriority,
									TaskHandle_t * const pxCreatedTask,
									TickType_t period,
									TickType_t capacity) PRIVILEGED_FUNCTION;
#endif /* configUSE_LLF_SCHEDULER */
